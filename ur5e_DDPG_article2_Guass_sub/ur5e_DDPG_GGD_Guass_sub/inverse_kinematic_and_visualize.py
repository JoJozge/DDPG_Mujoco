"""
用来测试MUJOCO自带的逆运动学代码的例程，目前测试得出的结论其自带的逆运动学雅可比矩阵不满足使用条件。
"""
import mujoco as mj
...
# smooth = True
#
df = pd.read_csv('./end_motion_path_csv/end_motion_table_{}.csv'.format(k), header=None)
...
xml_path = './universal_robots_ur5e_test/scene.xml'  #xml file (assumes this is in the same folder as this file)
...
def controller(model, data):
    ...
    # pass
    data.ctrl[0] = -3500 * (data.qpos[0] - yaim_list_qpos[0]) - 100 * (data.qvel[0] - 0)
    data.ctrl[1] = -3500 * (data.qpos[1] - yaim_list_qpos[1]) - 100 * (data.qvel[1] - 0)
    data.ctrl[2] = -3500 * (data.qpos[2] - yaim_list_qpos[2]) - 100 * (data.qvel[2] - 0)
    data.ctrl[3] = -3000 * (data.qpos[3] - yaim_list_qpos[3]) - 100 * (data.qvel[3] - 0)
    data.ctrl[4] = -3000 * (data.qpos[4] - yaim_list_qpos[4]) - 100 * (data.qvel[4] - 0)
    data.ctrl[5] = -3000 * (data.qpos[5] - yaim_list_qpos[5]) - 100 * (data.qvel[5] - 0)
...
while not glfw.window_should_close(window) and i<(max_episode):
    ...
    yaim_list_qpos = [theta1, theta2, theta3, theta4, theta5, theta6]
    ...
    if i == 0:
        position_Q = [x_ref[0], y_ref[0], z_ref[0]]
    else:
        position_Q = [x_ref[i - 1], y_ref[i - 1], z_ref[i - 1]]
    ...
    print(f'{jacp}jacp')
    # print(f'{J}J{k}')
    # print(f'self.sim.data.qacc{self.sim.data.qacc}{i}')
    ...
    # else:
    #     z_ref[i] = z_ref[i]
    dX = np.array([x_ref[i] - position_Q[0], y_ref[i] - position_Q[1], z_ref[i] - position_Q[2]])
