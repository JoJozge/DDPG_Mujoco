...
while not glfw.window_should_close(window) and i<(max_episode):
    ...
    while not done:
        ...
        action = (action + np.random.normal(0, 0.1, size=6)).clip(env.action_bound0[0], env.action_bound0[1])  # 给每一时间步的决策添加噪声，该语句是增强智能体探索能力的关键语句，有几率使得智能体跳出局部最优解
    ...
    if i % 1 == 0 and total_reward >= -5000:
        ...
        # print(df)
        df.to_csv('../../ur5e_DDPG_article2_0_1_20000csv/end_motion_path_csv/end_motion_table_{}.csv'.format(i),index=False)
        ...
        df.to_csv('../../ur5e_DDPG_article2_0_1_20000csv/joint_path_csv/joint_path_table_{}.csv'.format(i),index=False)

        reward_table = np.array(reward_table_list)
        df = pd.DataFrame(reward_table, columns=['episode', 'total_reward'])
        df.to_csv('../../ur5e_DDPG_article2_0_1_20000csv/reward_episode_csv/reward_episode_table_{}.csv'.format(i),index=False)
